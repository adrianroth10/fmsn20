...
The approximate log likelihood of $\alpha$ and $\beta$ where some spatial dependence is disregarded is
...
\begin{itemize}
  ...
  \item The $\alpha$ and $\beta$ are updated using Metropolis Hastings sampling.
    A certain
\end{itemize}<++>
...
\begin{itemize}
  ...
  \item The neighbourhood structure.
...
With 5 different number of classes, 5 number of principal components, 3 neighbourhood structures and two versions of the beta parameter there are 150 different $\sigma_{MH}^2$ to be optimized.
This is a pretty unreasonable task.
Instead of doing this we chose to try and find patterns in how the $\sigma_{MH}^2$ is connected to the acceptance rate, not good mixing since it requires a more subjective measure.
We say that a good $\sigma_{MH}^2$ is one that results in acceptance rate of $0.3 \pm 0.05$.
Comparing them in Table \dots and \dots there seems to be some connection with lower $\sigma_{MH}^2$ and more principal components.
Though this is not true for all combinations and from testing it is found that a lower $\sigma_{MH}^2$ not always results in larger acceptance rate which makes it hard to find a connection.
Instead an interesting connection is found between the neighbourhood structure of the DMRF model and a good $\sigma_{MH}^2$
The most interesting relation is found connected to the structure of the DMRF model.
A good $\sigma_{MH4}^2$ is found for a 4 neighbourhood structure.
For a 8 and 12 neighbourhood $\sigma_{MH4}^2 / 2^2$ and $\sigma_{MH4}^2 / 3^2$ respectively give good acceptance rate.
Somehow a good Metropolis Hastings sampling $\sigma_{MH}^2$ scales with the inverse quadratic of the number of neighbours.
This lowers the search space to ``only'' 50 different $\sigma_{MH}^2$.
At last we found a connection that taking half of the $\sigma_{MH}^2$ when using one $\beta$ parameter for each class is will also in general give a good acceptance rate.
\textcolor{white}{Bam}, only 25 combinations left which were optimized in Table \dots.
